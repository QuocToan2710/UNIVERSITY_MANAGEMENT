...
- [x] **1.2. Cổng Đăng ký Tín chỉ Trực tuyến & Quản lý Ghi danh (Student Course Registration & Class Enrollment) - [ĐÃ HOÀN THÀNH]:**
  - **Mô tả:** Sinh viên chủ động đăng ký/hủy học phần trực tuyến trong đợt mở đăng ký theo kỳ (`/course-registration`).
  - **Ràng buộc nghiệp vụ (Business Constraints):**
    - Kiểm tra xung đột / trùng lịch học (`ClassSchedule` dayOfWeek & overlapping hours).
    - Kiểm tra sĩ số tối đa của lớp học phần (`currentCapacity < maxCapacity`).
    - Kiểm tra đăng ký trùng lặp lớp học phần.
    - Giới hạn số tín chỉ tối đa trong học kỳ (`24 TC`).
    - Gửi thông báo realtime đến sinh viên khi đăng ký/hủy môn thành công.
  - **Dành cho Quản trị viên & Giảng viên:**
    - Modal quản lý sinh viên trong lớp học phần (`EnrollmentManagementModal` trên `/courses`).
    - Xem danh sách SV hiện tại, xuất Excel danh sách lớp.
    - Gán toàn bộ sinh viên từ Lớp sinh hoạt (`ClassGroup`) vào Lớp học phần chỉ bằng 1 thao tác (Batch insert).
    - Thêm từng sinh viên vào lớp học phần qua danh sách chọn lọc.

- [x] **1.3. Quản lý Tài chính & Tra cứu Học phí Sinh viên (Tuition & Financial Management) - [ĐÃ HOÀN THÀNH]:**
  - **Mô tả:** Hệ thống theo dõi công nợ học phí, biểu giá tín chỉ (450.000 đ/TC) và lịch sử thu chi (`/finance/tuition`).
  - **Sinh viên:** Phiếu báo học phí cá nhân chi tiết từng lớp học phần đã đăng ký, số tiền phát sinh, số tiền đã đóng, công nợ còn lại, hạn nộp và xuất phiếu báo Excel.
  - **Admin / Kế toán:** Dashboard KPI tài chính 4 chỉ số (Tổng học phí, Tổng thực thu, Tổng công nợ, Tỷ lệ thu hồi %), bộ lọc học kỳ, năm học, lớp sinh hoạt, trạng thái nộp tiền, tìm kiếm sinh viên, Modal ghi nhận thu tiền học phí trực tiếp (Chuyển khoản, Tiền mặt, POS) và xuất báo cáo công nợ Excel.

- [x] **1.4. Quản lý Điểm danh & Chuyên cần (Attendance & Absence Warning) - [ĐÃ HOÀN THÀNH]:**
  - **Tài liệu đặc tả:** Chi tiết tại [`ATTENDANCE_MODULE_SPEC.md`](file:///D:/My%20Project/UNIVERSITY_MANAGEMENT/university-management/docs/ATTENDANCE_MODULE_SPEC.md).
  - **Giảng viên & Admin:** Giao diện điểm danh buổi học trực quan (`/teaching/attendance`), sinh tự động 15 buổi theo TKB hoặc thêm buổi học bù, điểm danh nhanh 1-click (Có mặt / Đi muộn / Có phép / Vắng), chốt sổ điểm danh và xuất Excel danh sách buổi.
  - **Tự động tính Điểm & Cảnh báo:** Hệ thống tự động tính điểm chuyên cần (thang điểm 10) cập nhật vào `Enrollment.attendanceScore`, tính tỷ lệ vắng $\%$, gửi Notification cảnh báo nguy cơ ($10\% - 20\%$) và áp dụng chế tài **CẤM THI** khi vắng $> 20\%$.
  - **Sinh viên:** Tra cứu chuyên cần cá nhân (`/student/attendance`), theo dõi thanh tiến độ % vắng và nhật ký từng ngày học.
  - **Báo cáo cấm thi:** Báo cáo danh sách sinh viên cấm thi toàn trường (`/admin/attendance-reports`) kèm xuất Excel nộp Hội đồng thi.
...
- [x] **2.3. Quản trị Hệ thống, Phân quyền Vai trò & Quét Endpoint API Tự động (RBAC & Dynamic Permissions) - [ĐÃ HOÀN THÀNH]:**
  - Cụm chức năng Quản trị hệ thống (`/users`, `/roles`): Quản lý tài khoản, gán quyền và quản lý ma trận Roles (`ADMIN`, `TEACHER`, `STUDENT`, `STAFF`, v.v.).
  - Giao diện Phân quyền đa cấp theo từng Phân hệ (`Identity`, `Schedule`, `Grade`, `Tuition`, `Masterdata`, `Notification`), hỗ trợ tìm kiếm quyền và chọn nhanh quyền hạn.
  - Bộ quét endpoint API động (`EndpointAutoScanner` & `DynamicApiAuthorizationManager`) tự động đồng bộ 189+ API permissions vào database.
