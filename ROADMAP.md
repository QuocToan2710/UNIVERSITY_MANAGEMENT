...
- [ ] **Task 3.3: Xử Lý Xung Đột & Tranh Chấp Sĩ Số Khi Đăng Ký Môn (Concurrency & Optimistic Lock) - [ƯU TIÊN CAO]**
  - Thêm `@Version` vào `SubjectClass` hoặc dùng Redis Lock để đảm bảo không bị vượt quá sĩ số lớp (`currentCapacity > maxCapacity`) khi hàng trăm sinh viên cùng đăng ký 1 slot cuối cùng.
